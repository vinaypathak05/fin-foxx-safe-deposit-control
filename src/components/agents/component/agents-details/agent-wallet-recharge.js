...
import {
  ...
  LABEL_POSITION_TOP,
  AGENT_INSENTIVE,
  AGENT_SECURITY_AMOUNT,
  AGENT_FIRST_RECHARGE,
...
class WalletRecharge extends Component {
  ...
  onSubmitForm = (data) => {
    ...
    // if (
    //   agentAllRecharges &&
    //   agentAllRecharges.data &&
    //   agentAllRecharges.data.length == 0
    // ) {
    ...
    // values.balance =
    //   parseFloat(selectedAgent.agentDetails.balance) + parseFloat(fullamount);
    // values.earnings =
    //   parseFloat(selectedAgent.agentDetails.earnings) + parseFloat(insentive);
    ...
    this.props.agentWalletRecharge(session, values, (response) => {
      ...
      if (response.success == 1) {
        this.props.showSuccess(
          LocaleStrings.agents_detail_wallet_add_form_success
        );
          ...
          <div className="modal-body">
            ...
            agentAllRecharges.data.length == 0 ? (
              <div className="mb-4">
                ...
                ></i>
                &nbsp;
                {LocaleStrings.agents_detail_wallet_text_security_money}: Rs.
                {AGENT_SECURITY_AMOUNT}
              </div>
            ) : (
              ""
...
function validate(values, ownProps) {
  ...
  if (!amountpaid || amountpaid === "") {
    ...
    errors["amountpaid"] =
      LocaleStrings.agents_detail_wallet_validation_invalid_amount;
  } else if (
    agentAllRecharges &&
    agentAllRecharges.data &&
    agentAllRecharges.data.length == 0 &&
    amountpaid &&
    amountpaid < AGENT_FIRST_RECHARGE
  ) {
    errors["amountpaid"] =
      LocaleStrings.agents_detail_wallet_validation_security_amount_required;
  }
