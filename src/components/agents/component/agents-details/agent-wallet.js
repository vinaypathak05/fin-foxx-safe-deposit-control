...
import {openWalletRechargeModal,fetchSingleAgentWalletRecharges,fetchSingleAgent} from '../../action';
...
class AgentWallet extends Component {
    ...
    paginationCallback = (pageNumber) => {
        ...
        this.props.fetchSingleAgentWalletRecharges(session, selectedAgent.agentDetails.agentid, pageNumber, (response) => {
    ...
    listRender = () => {
        ...
        return _.map(this.props.agentWalletRecharges.data, (item, index) => {
            return <RenderList key={index} printList={item} pagination={this.paginationCallback} currentPage={pageNumber} />;
        });
    }
                    ...
                    <Card className="shadow">
                        <CardHeader className="border-0">
                            <Row>
                                ...
                                {selectedAgent && selectedAgent.agentDetails && selectedAgent.agentDetails.approvalstatus === 'approved' && selectedAgent.agentDetails.status === 'active' ? 
                                    <Col md={6} className="text-right">
                                        <Button color="primary" size="sm" type="button" className="ml-3" onClick={this.openModal}>{LocaleStrings.button_recharge}</Button>
                                    </Col>
                                :   
                                    ''
                                }
                            ...
                            {modalStatus && modalStatus.showModal ? 
                                <WalletRecharge finishOperationsCallback={this.additionalCallback} agentAllRecharges={agentWalletRecharges}/>
                            :
                                ''
                            }
                        </CardHeader>
                        {agentWalletRecharges && agentWalletRecharges.data ?
                            <div>
                                {agentWalletRecharges.data && agentWalletRecharges.data.length > 0 ?
                                    <Table className="align-items-center table-flush min-height-135" responsive>
                                        <thead className="thead-light">
                                            <tr>
                                                <th scope="col">{LocaleStrings.agents_detail_table_th_recharge_amount}</th>
                                                <th scope="col">{LocaleStrings.agents_detail_table_th_earning_amount}</th>
                                                <th scope="col">{LocaleStrings.agents_detail_table_th_recharge_date}</th>
                                                <th scope="col" />
                                            </tr>
                                        </thead>
                                        <tbody>
...
export default connect(mapStateToProps, {openWalletRechargeModal,fetchSingleAgentWalletRecharges,fetchSingleAgent})(AgentWallet);