...
class ApproveAgents extends BaseComponent {
    ...
    onSubmitForm = (values) => {
        ...
        let data = {agentid: values.agentid, approvalstatus: values.approvalstatus}
        ...
        return (
            <Modal className="" isOpen={modalStatus.showModal == true ? true : false}>
                <div className="modal-header">
                    <h2 className="modal-title" id="modal-title-default"> {LocaleStrings.agents_approve_madal_title_approve_agent}</h2>
                    <button
                        aria-label="Close"
                        className="close"
                        data-dismiss="modal"
                ...
                <form onSubmit={handleSubmit(this.onSubmitForm)} encType="multipart/form-data">
                    <div className="modal-body">
                        <div className="form-group row">
                            <label className="custom-label col-sm-12">{LocaleStrings.agents_approve_label_select}*</label>
...
export default connect(mapStateToProps, {openAgentApproveModal,approveAgent,showSuccess,showError}) (reduxForm({
    validate,
    form:'AgentForm',
