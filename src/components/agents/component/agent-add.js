...
import {openAgentCreateModal,agentKycFiles,addAgent,addAgentWallet} from '../action';
        ...
        if(!editMode) {
            ...
            this.props.addAgent(session, values, (response) => {
                if(response.success == 1) {
                    let wvalue = {agentidfk: response.data.insertId, balance: '0.00'};

                    this.props.addAgentWallet(session, wvalue, (wres) => {
                ...
                <form onSubmit={handleSubmit(this.onSubmitForm)} encType="multipart/form-data">
                    <div className="modal-body">
                        <Tabs className="branding-tabs mt-0" id="speaker-from-tab" activeKey={this.state.selectedTab} onSelect={this.handleSelect}>
                            <Tab className="pt-3" eventKey={1} title={LocaleStrings.agents_madal_tab_text_basic_details}>
                                <AgentForm {...this.props}/>
                            </Tab>
                            <Tab className="pt-3" eventKey={2} title={LocaleStrings.agents_madal_tab_text_bank_details}>
                                <BankForm {...this.props}/>
                            </Tab>
                            <Tab className="pt-3" eventKey={3} title={LocaleStrings.agents_madal_tab_text_docs_details}>
...
export default connect(mapStateToProps, {openAgentCreateModal,agentKycFiles,addAgent,addAgentWallet,showSuccess,showError}) (reduxForm({
    validate,
    form:'AgentForm',
...
class KYCFileUpload extends BaseComponent {
    ...
    onFilesDrop = (index, files) => {
        ...
        uploadedAgentKycFiles[index].filename = files.filename ? files.filename : "";
