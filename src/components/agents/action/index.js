...
export function fetchAllAgents(session, callback) {
    ...
    return (dispatch) => {
        getRequest(url, session, dispatch, (response) => {
            ...
            dispatch({type:ALL_AGENTS_LISTS, payload: {data:response.resource, count:response.count ? response.count : ''}});
            callback({success: 1});
        }, (error) => {
...
export function addAgentWallet(session, values, callback) {
    var url = `${BASE_URL}/api/addagentwallet`;
    var body = new URLSearchParams(values);
    // console.log('body : - ', body)
...
export function fetchSingleAgent(session, id, callback) {
    ...
    return (dispatch) => {
        getRequest(url, session, dispatch, (response) => {
            // console.log('response : - ', response)
            dispatch({type: SELECTED_AGENT, payload: {agentDetails: response.resource[0]}});
            callback({success: 1});
        }, (error) => {
            // callback({success: 0, error: error});
        });
...
export function fetchSingleAgentWalletRecharges(session, id, pageCount = 0, callback) {
    var perPage = 10;
    var offset = pageCount <= 1 ? 0 : (pageCount-1) * perPage;
    ...
    return (dispatch) => {
        postMultipartRequest(url, body, session, dispatch, (response) => {
            // console.log('response : - ', response)
            dispatch({type: AGENT_WALLET_RECHARGES, payload: {data: response.resource, count:response.count ? response.count : ''}});
            callback({success: 1});
        }, (error) => {
            // callback({success: 0, error: error});
        });
...
export function fetchSingleAgentCustomers(session, id, pageCount = 0, callback) {
    var perPage = 10;
    var offset = pageCount <= 1 ? 0 : (pageCount-1) * perPage;
    ...
    return (dispatch) => {
        postMultipartRequest(url, body, session, dispatch, (response) => {
            // console.log('response : - ', response)
            dispatch({type: AGENT_CUSTOMERS, payload: {data: response.resource, count:response.count ? response.count : ''}});
            callback({success: 1});
        }, (error) => {
            // callback({success: 0, error: error});
        });
