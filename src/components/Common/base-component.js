...
export class BaseComponent extends Component {
    ...
    renderFieldText(field){
        ...
        var divClassName=`form-group row ${(field.meta.touched && field.meta.error) ? 'has-danger' : ''}`;
        ...
        return(
            <div className={divClassName}>
                <label className={labelClasses}>
                    {field.label}<span>{(field.mandatory === 'true')?'*':''}</span>
                </label>
                ...
                <div className={inputClasses}>
                    ...
                    />
                    <div className="text-help label-text-help">{field.meta.touched?field.meta.error:''}</div>
                </div>
            </div>
        );
    }
    ...
    renderHiddenFieldTextShowError(field){
        ...
        return(
            <>
                ...
                <div className="form-group text-help label-text-help pl-3">{field.meta.error?field.meta.error:''}</div>
            </>
        );
    }
    ...
    renderFieldSelect(field) {
        ...
        return(
            <div className={divClassName}>
                ...
                <div className={inputClasses}>
                    <select className="form-control" {...field.input} disabled={field.disabled ? true : false}>
                        <option value='' key='key_selectbox'>{field.placeholder}</option>
                        {field.opts.map((object,index) => {
                            return <option key={`${object.value}-${index}`} value={object.value} >{object.label}</option>
                        })}
                    </select>
                    <div className="text-help label-text-help">{field.meta.touched ? field.meta.error :''}</div>
                </div>
    ...
    renderFieldMultipleCheckbox(field) {
        ...
        var divClassName=`form-group row ${(meta.touched && meta.error) ? 'has-danger' : ''}`;
        ...
        return(
            <div className={divClassName}>
                <label className={labelClasses}>
                    {label}<span>{(field.mandatory === 'true')?'*':''}</span>
                </label>
                ...
                <div className={inputClasses}>
                    ...
                    <div className="text-help label-text-help">{meta.touched?meta.error:''}</div>
                </div>
            </div>
        );
    }
    ...
    renderFieldRadio(field) {
        ...
        return (
            <div style={{marginRight: 10}}>
                <label className="custom-container-radio liveqa-label" style={{marginBottom: 5}}>
                    {field.label}
                    <input
                        {...field.input}
                        type={field.type}
    ...
    renderAutoComplete(field) {
        ...
        if(defaultVal && !_.isEmpty(defaultVal)) {
            if(Array.isArray(defaultVal) || (defaultVal instanceof Object || typeof defaultVal === "object" )) {
                selected = defaultVal
            }
            else {
                ...
                values.forEach(item => {      
                    var added = _.filter(field.options, obj => {return obj.value == item});
