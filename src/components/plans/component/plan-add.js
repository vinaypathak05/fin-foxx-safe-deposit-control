        ...
        if(!editMode) {
            ...
            this.props.validatedPlanName(session, values, (vres) => {
                ...
                if(vres.success == 1) {
                    ...
                    this.setState({loading:false});
                    this.props.showError(LocaleStrings.plans_form_validation_plan_name_exist);
                }
                else {
                    this.setState({loading:false});
                    this.props.showError(COMMON_FAIL_MESSAGE);
        ...
        return (
            <Modal className="" isOpen={modalStatus.showModal == true ? true : false}>
                <div className="modal-header">
                    <h2 className="modal-title" id="modal-title-default"> {edit ? LocaleStrings.plans_madal_title_edit : LocaleStrings.plans_madal_title_add}</h2>
                    <button
                        aria-label="Close"
                        className="close"
                        data-dismiss="modal"
...
function validate(values) {
    ...
    if(!planduration || planduration === '') {
        ...
        errors['planduration'] = LocaleStrings.plans_form_validation_plan_duration_min;
    }
...
export default connect(mapStateToProps, {openPlanCreateModal,validatedPlanName,addNewPlan,showSuccess,showError}) (reduxForm({
    validate,
    form:'PlanForm',
