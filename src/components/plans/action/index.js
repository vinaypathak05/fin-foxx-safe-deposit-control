...
export function fetchAllPlans(session, callback) {
    ...
    return (dispatch) => {
        getRequest(url, session, dispatch, (response) => {
            ...
            dispatch({type:ALL_PLANS_LISTS, payload: {data:response.resource, count:response.count ? response.count : ''}});
            callback({success: 1});
        }, (error) => {
...
export function fetchPlans(session, pageCount = 0, callback) {
    ...
    return (dispatch) => {
        getRequest(url, session, dispatch, (response) => {
            // console.log('response : - ', response)
            dispatch({type:PLANS_LISTS, payload: {data:response.resource, count:response.count ? response.count : ''}});
            callback({success: 1});
        }, (error) => {
            // callback({success: 0, error: error});
        });
...
export function validatedPlanName(session, values, callback) {
    ...
    return (dispatch) => {
        getRequest(url, session, dispatch, (response) => {
            // console.log('response : - ', response)
            if(response.success == 1 && response.resource && response.resource.length == 0) {
                callback({success: 1});
            }
            else {
                callback({success: 2});
