...
import {activateDeactivateCustomer,openCustomerPaymentModal,openCustomerApproveModal,selectedCustomerDetails} from '../action';
...
class CustomersList extends Component {
    ...
    collectAmount = (e) => {
        ...
        this.props.openCustomerPaymentModal({showModal: true, callfrom: this.props.callfrom, details: this.props.printList});
    }
    ...
    approveAction = (e) => {
        ...
        this.props.openCustomerApproveModal({showModal: true, details: this.props.printList});
    }
    ...
    activateDeactivate = (e) => {
        ...
        let values = {customerid: printList.customerid, status: printList.status == 'active' ? 'deactivated' : 'active'};
        this.props.activateDeactivateCustomer(session, values, (response)=>{
            if(response.success === 0) {
                this.props.showError(COMMON_FAIL_MESSAGE);
            } else {
                this.props.showSuccess(printList.status == 'active' ? LocaleStrings.customers_alert_deactivated : LocaleStrings.customers_alert_activated);
                ...
                <td>
                    <UncontrolledDropdown>
                        ...
                        <DropdownMenu className="dropdown-menu-arrow" right>
                            {printList.status === 'active' && printList.approvalstatus == 'approved' ?
                                <DropdownItem
...
export default connect(mapStateToProps, {activateDeactivateCustomer,openCustomerPaymentModal,openCustomerApproveModal,selectedCustomerDetails,showSuccess,showError})(CustomersList);
