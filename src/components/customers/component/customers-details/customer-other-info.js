...
class CustomerOtherInfo extends Component {
    ...
    approveAction = (e) => {
        this.props.openCustomerApproveModal({showModal: true, details: this.props.selectedCustomer.details});
    }
    ...
    render() {
        ...
        if(selectedCustomer && selectedCustomer.details && selectedCustomer.details.userpic) {
            let userpic = `${BASE_IMAGES_URL}/${selectedCustomer.details.userpic}`;
            kycImgs.push({src: userpic})
        }
        if(selectedCustomer && selectedCustomer.details && selectedCustomer.details.aadhaarfrontpic) {
            let aadhaarfrontpic = `${BASE_IMAGES_URL}/${selectedCustomer.details.aadhaarfrontpic}`;
            kycImgs.push({src: aadhaarfrontpic})
        }
        if(selectedCustomer && selectedCustomer.details && selectedCustomer.details.aadhaarbackpic) {
            let aadhaarbackpic = `${BASE_IMAGES_URL}/${selectedCustomer.details.aadhaarbackpic}`;
            kycImgs.push({src: aadhaarbackpic})
        }
        if(selectedCustomer && selectedCustomer.details && selectedCustomer.details.bankdetailspic) {
                    ...
                    <Card className="shadow">
                        <CardHeader className="border-0">
                            <Row>
                                ...
                                {selectedCustomer && selectedCustomer.details && selectedCustomer.details.status == 'active' && (selectedCustomer.details.approvalstatus == 'submitted' || selectedCustomer.details.approvalstatus == 'onhold') ?
                                    <Col md={6} className="text-right">
                                        <Button color="primary" size="sm" type="button" className="ml-3" onClick={this.approveAction}>{LocaleStrings.button_approve}</Button>
                                    </Col>
                                :
                                    ''
                                }
                        ...
                        <CardBody>
                            {kycImgs.length > 0 ? 
                                <Button color="primary" onClick={this.viewKyc}>{LocaleStrings.button_view_kyc}</Button>
                            :
...
export default connect(mapStateToProps, {openCustomerApproveModal})(CustomerOtherInfo);