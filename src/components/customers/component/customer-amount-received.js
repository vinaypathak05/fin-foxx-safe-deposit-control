...
class CUstomerPaymentReceived extends Component {
  ...
  onSubmitForm = (values) => {
    ...
    values.balance =
      parseFloat(modalStatus.details.balance) - parseFloat(values.amountpaid);
    values.paidtilldate =
      parseFloat(modalStatus.details.paidtilldate) +
      parseFloat(values.amountpaid);
    values.planleftamount =
      parseFloat(modalStatus.details.planleftamount) -
      parseFloat(values.amountpaid);
    // console.log('Values: -', values)
  ...
  render() {
    ...
    var customerPlanPaymentLastDate = moment(planActivatedon).add(
      parseInt(modalStatus.details.planduration),
      "d"
    );
    var customerPlanExpireDate = customerPlanPaymentLastDate.format(
      "DD-MM-YYYY"
    );
          ...
          <div className="modal-body">
            ...
            <div className="mb-4">
              <i
                ...
                style={{ fontSize: 20 }}
              ></i>
              &nbsp; Customer's plan start date:{" "}
              {converDateIntoLocalDate(modalStatus.details.planactivatedate)}
            </div>
            <div className="mb-4">
              <i
                className="fa fa-info-circle text-info"
...
function validate(values, ownProps) {
  ...
  if (!amountpaid || amountpaid === "") {
    ...
    if (
      ...
      amountpaid &&
      parseFloat(amountpaid) > parseFloat(ownProps.modalStatus.details.balance)
    ) {
      errors["amountpaid"] =
